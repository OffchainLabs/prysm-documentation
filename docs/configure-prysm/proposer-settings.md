...
| `gas_limit` | The gas limit your validator advertises as its preference for blocks built on its behalf. **New in v2 at this level.** Leave it unset to follow the network's scheduled gas limit ([EIP-8261](https://eips.ethereum.org/EIPS/eip-8261)), or the chain default shipped in your Prysm release when no schedule entry is active — the [command-line options](/configure-prysm/command-line-options.md) reference is generated per release, and the default it shows for `--suggested-gas-limit` is that same value. Set it only to deliberately opt out of the network schedule; Prysm logs a warning once per epoch when your explicit value is above or below the scheduled value. In v1 the gas limit lived inside `builder`; that placement is legacy and stops applying at the Gloas fork. |
